...
export default {
  ...
  methods: {
    ...
    onStockChanged() {
      // Actualizar inmediatamente con pequeño delay para asegurar que la API esté actualizada
      setTimeout(() => {
  ...
  mounted() {
    ...
    // Obtener datos del usuario desde localStorage (lectura rápida)
    this.usuarioLogueado = JSON.parse(localStorage.getItem('user')) || {};

    // Cargar alertas de stock solo si está autenticado
...
.navbar-icon-btn.has-alerts {
  color: #ef4444;
  animation: pulse-bell 1.5s infinite;
}

.navbar-icon-btn.has-alerts:hover {
  color: #dc2626;
}
